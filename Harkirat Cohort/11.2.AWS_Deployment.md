...
And now a backend application is running on a server. Now lets say the server is running on port 8080 but you havent opened the port 8080 in the security group, so you cannot access it from outside. So go to the security groups section in your machine's aws console, and in security groups and add change inbound rules and add a custom TCP rule with port 8080 and source as anywhere from ipv4 and ipv6.

### Try hitting the Server
- You have an ip/DNS that you can hit to access your ec2 server. So ip and that link is like http://ec2-65-0-180-32.ap-south-1.compute.amazonaws.com:8080 which is ugly.
- If it was on port 80 then you dont have to mention :8080, you can hit the domain name with http, or if it was on port 443 you didnt need to mention the port, you can directly hit the domain name with https.
- See one thing, you can simply do app.listen(80) or 443 and when hitting url you need not to write :port in the url link. You shouldnt do it. Your nodejs process should never listen on port 80 or 443. 
- If you just change app.listen(80) then you will see error, that says "Error: listen EACCES: permission denied 0.0.0.0:80". Port 80, 443, 22 are privileged ports, OS doesnt allow you to run a process on these ports. Although if you give the node some very heavy access then you maybe able to run it on port 80.


## Reverse Proxy
Now lets you give the node some very heavy access then you maybe able to run it on port 80 some how, now you need not to mention the port number, now lets say you have to deploy another backend here, you cannot have two process on port 80.
On the ip/machine you bought, you want be1.varun.com and be2.varun.com both to run, but you cant run both to port 80, which is why it make sense to use Reverse Proxy. So be1 running on 3000 and be2 running on 3001 to make this happen we use reverse proxy.
...
**Proxy:**
so lets say your college has blocked access to badweb.com and you go to chrome and you write a proxy url there and your req is not going to the badweb.com, rather it goes to the proxy server which is not blocked by your college and then that server sends the req to the blocked url. VPNs are also type of proxy, If you are in india and connect to vpn in japan and all you see is blurred videos wtf (very bad joke).
Instead of your request going directly to badweb.com, it first goes to the proxy server. The proxy server then forwards your request to badweb.com on your behalf. The proxy server receives the response from badweb.com. It then sends that response back to you. This way, your browser displays the content of badweb.com, but the request never went directly to that site. Since your college has blocked badweb.com, the proxy server acts as a middleman, allowing you to bypass that restriction. VPN not only routes your web traffic through a server but also encrypts it, providing privacy and security while bypassing geographical or network restrictions. So thats **forward proxy**.
...
Similar to how a VPN routes your request through a proxy server to bypass restrictions which was forward proxy, a reverse proxy (a user is sending a req to nginx which is getting proxied to a process) routes requests to different internal services based on the URL, allowing multiple applications to run on the same machine without port conflicts. In forward proxy we dont initiate req directly to server but to the proxy server, here the user initiates the req direct to server but the server is a reverse proxy which then routes the req to the correct process as per URL.
...
Even load balancers that takes millions of requests and distributes them across multiple servers use reverse proxy. NGINX is a popular choice for reverse proxying due to its performance and flexibility.
...
- Try visiting the website. You will see nginx (port 80 should be opened in your security group).

Steps to do so:
- copy the ip of your ec2 machine. Go to your domain provider dashboard, and go to dns and add entry eg be1.varuntd.com and select A and paste the ip of the instance. And create entry for be2.varuntd.com too for same instance ip.
- Then ssh to your instance and run 2 processes on different ports, lets say 3000 and 3001.
- Now create the reverse proxies. You need not to know the full syntax to do so in nginx file, you can google, below is the basic syntax you need to know.
