...
> (rate limiting doesnt necessarily prevent DDoS protection) **DDoS Protection:** A DDoS attack involves overwhelming a site with a flood of traffic from multiple sources, which can make the website unavailable. DDoS protection mechanisms detect unusual traffic flows and can filter out malicious traffic, helping to keep the service operational despite the attack.


So you rate limit based on IP. Actually you shouldnt because eg there is a college and there are internal ips given to them in the college, but the whole college is using the external ip. So if one person in the college is making many requests then the whole college will be blocked. As the server will see the same ip for all the requests. So you should rate limit based on user id or api key. 

Instead of directly having all the requests on the server we use load blancer. Why? Because someone can req many times, like running a for loop million time with fetch in it.
...
But how do attacker have access to so many machines?
...
Now how do we protect against DDoS?
There are some really good services like cloudflare, AWS Shield, GCP Armor that provide DDoS protection. So the req to your server goes via them and they figure out if its a legit req or not. Also these providers expects you to bring your domain to them. You transfer your domain to cloudflare or you buy them. Whenever you set your dns records, you proxy them via cloudflare. It will handle the DDoS prevention for you. So if youre creating your website, want to prevent DDoS, put it in front of cloudflare.

Capchas arent really a good way to prevent a DDoS. Captchas are a great solution to making sure the request was sent by a human and not by a machine
...
You cannot solve a captcha via nodejs process, people created captchas are trying their best so that it cant be automatically done via nodejs process, a browser is required.
