...
So high level, if you want to buid any fullstack application then, when you want a lot of contributors to work on the same codebase, you want to make sure that the codebase is running in a similar environment for everyone. Whenever new developer joins the team, they can run the project in a single command. Docker provides you a easy way to setup the project locally, easy way to deploy the project accross different environments, eg today ec2, tomorrow gcp, etc... You can run on any OS, any machine, like if you have a fresh machine, and you wanna run a fullstack project like propayn then you dont have to install node, npm, postgres, etc, you just have to install docker and run a single command and it will internally start those things and run the whole project in a container. So it makes it easy to run the project locally, deploy the project in production, and also lets you run other people's code easily.
So its a good idea to containerise your application and make it isolated from the host machine.
